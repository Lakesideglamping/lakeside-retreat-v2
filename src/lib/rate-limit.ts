 ...
 *
 * Fail-open policy: if Postgres is unreachable or slow enough to throw,
 * we log and allow the request. A broken rate limiter must never DoS the
 * caller (especially auth routes) — availability beats perfect throttling.
 */
...
export async function clearRateLimit(keyPrefix: string): Promise<void> {
  try {
    ...
    logger.error("rate-limit DB failure on clear, ignoring (fail-open)", {
      keyPrefix,
...
export async function checkRateLimit(
  ...
  try {
    ...
    // DB unreachable or slow enough to throw — allow the request through.
    // Throwing here would 500 the caller (e.g. /api/admin/login), turning a
    // DB blip into an outage for every protected route.
    logger.error("rate-limit DB failure, allowing request (fail-open)", {
      key,
      error: err instanceof Error ? err.message : String(err),
    });
    return { success: true, remaining: max - 1 };
